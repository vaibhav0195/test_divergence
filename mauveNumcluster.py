...
if __name__ == '__main__':
    ...
    # std_p = np.diag(np.random.uniform(size = dimensions_inp, low = 0, high = 1))
    std_p = make_semidef_mat(dimensions_inp, 0, 1)
    mean_q = [getrandomNumber(0, 1) for i in range(dimensions_inp)]
    # mean_q = np.random.uniform(size=dimensions_inp, low=0, high=5)
    # std_q = np.diag(np.random.uniform(size=dimensions_inp, low=0, high=1))
    std_q = make_semidef_mat(dimensions_inp, 0, 1)
    sample_size = 1000
    maxNumCluster = 100
    numClusters = [2] + list(range(10,10*(maxNumCluster+1),10))
    for numCluster in numClusters:
        ...
        for bootstap_idx in range(5):
            ...
            actualDive = gaussian_divergence_ndims(mean_p, mean_p, std_p, std_p)
            # divergence = estimate(p_feat_sampled, q_feat_sampled,n_jobs=4)
            # divergence = skl_efficient(p_feat_sampled, q_feat_sampled, k=13)
            # num_buckets = std_sampled_p.shape[0]
            # if num_buckets == 1:
            #     num_buckets += 1
            divergenve_out = get_divergence_value(p_feat_sampled,q_feat_sampled,numCluster)
            # divergenve_out_2 = get_divergence_value(p_feat_sampled,q_feat_sampled,int(0.5*sample_size))
            # num_buckets = max(2, int(round(min(p_feat_sampled.shape[0], q_feat_sampled.shape[0]) * 0.1)))
            # divergenve_out_3 = get_divergence_value(p_feat_sampled,q_feat_sampled,num_buckets)
            mauve_ori.append(divergenve_out)
            # mauve_updated.append(divergenve_out_2)
            # mauve_updated_2.append(divergenve_out_3)
        ...
        # Y_mauve_new2.append([np.mean(mauve_updated_2),np.std(mauve_updated_2)])
