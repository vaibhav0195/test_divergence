...
def compute_mauve(
    ...
    * ``out.divergence_curve`` contains the points in the divergence curve. It is of shape (m, 2), where m is ``divergence_curve_discretization_size``
    ...
    # Acutal binning
    t1 = time.time()
    p, q,p_labels,q_labels, kmeans_model, pca, maxvarienceIdx, normRet = cluster_feats(p_features, q_features,
    ...
    to_return = SimpleNamespace(
        p_hist=p, q_hist=q, p_labels=p_labels, q_labels=q_labels
    ,divergence_curve=divergence_curve,
        mauve=mauve_score,
        frontier_integral=fi_score,
        num_buckets=num_buckets,kmeans_model = kmeans_model, pca=pca, maxvarienceIdx=maxvarienceIdx, normRet = normRet
    )
    return to_return
...
def compute_divergence(
        p_features=None, q_features=None,
        num_buckets='auto', pca_max_data=-1, kmeans_explained_var=0.9,
        kmeans_num_redo=5, kmeans_max_iter=500, device_id=-1, max_text_length=1024,
        divergence_curve_discretization_size=25, mauve_scaling_factor=5,
        verbose=False, seed=25, batch_size=1, use_float64=False,
):
    ...
    * ``out.divergence_curve`` contains the points in the divergence curve. It is of shape (m, 2), where m is ``divergence_curve_discretization_size``
    ...
    """
    p_features = get_features_from_input(
        p_features, None, None, None, max_text_length,
        device_id, name="p", verbose=verbose, batch_size=batch_size, use_float64=use_float64,
    )
    q_features = get_features_from_input(
        q_features, None, None, None, max_text_length,
        device_id, name="q", verbose=verbose, batch_size=batch_size, use_float64=use_float64,
    )
    if num_buckets == 'auto':
        # heuristic: use num_clusters = num_generations / 10
    ...
    # Divergence curve and mauve
    # mixture_weights = np.linspace(1e-6, 1-1e-6, divergence_curve_discretization_size)
    mixture_weights = [0.000001,0.999999]
    divergence_curve = []
    for w in mixture_weights:
        r = w * p + (1 - w) * q
        divergence_curve.append([kl_multinomial(q, r), kl_multinomial(p, r)])
    divergence_curve = np.exp(-mauve_scaling_factor * np.asarray(divergence_curve))
    # divergence_curve = np.asarray(divergence_curve)
    return divergence_curve

def compute_divergence_new(
        p_features=None, q_features=None,
):
    p_features = np.asarray(p_features)
    q_features = np.asarray(q_features)
    # Divergence curve and mauve
    # mixture_weights = np.linspace(1e-6, 1-1e-6, divergence_curve_discretization_size)
    mixture_weights = [0.000001,0.999999]
    divergence_curve = []
    for w in mixture_weights:
        r = w * p_features + (1 - w) * q_features
        # print(p_features.shape)
        idxs_p = np.logical_and(p_features != 0, r != 0)
        idxs_q = np.logical_and(q_features != 0, r != 0)
        p = []
        r_p = []
        q = []
        r_q = []
        for iTemp,idx_p in enumerate(idxs_p):
            if idx_p:
                p.append(p_features[iTemp])
                r_p.append(r[iTemp])
        for iTemp,idx_q in enumerate(idxs_q):
            if idx_q:
                q.append(q_features[iTemp])
                r_q.append(r[iTemp])
        divergencep = estimate(p, r_p, n_jobs=4)
        divergenceq = estimate(q, r_q, n_jobs=4)
        divergence_curve.append([divergencep, divergenceq])
    # divergence_curve = np.exp(-mauve_scaling_factor * np.asarray(divergence_curve))
    # divergence_curve = np.asarray(divergence_curve)
    return np.asarray(divergence_curve)
...
def cluster_feats(p, q, num_clusters,
    ...
    p_bins = np.histogram(p_labels, bins=num_clusters,
                          range=[0, num_clusters], density=True)[0]
    return p_bins / p_bins.sum(), q_bins / q_bins.sum(), p_labels, q_labels, kmeans, pca, idx+1, norm
